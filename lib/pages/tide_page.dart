...
class _TidePageState extends State<TidePage>
          ...
          slivers: [
            ...
            SliverPadding(
              ...
              sliver: SliverGrid(
                ...
                delegate: SliverChildListDelegate([
                  ...
                  _buildConditionWindForSelected(),
                  _buildConditionWavesForSelected(),
            ...
            SliverPadding(
              ...
              sliver: SliverGrid(
                ...
                delegate: SliverChildListDelegate(
                  _data.tideEvents
                      .map((event) => _buildTideEventCard(event))
                      .toList(),
                ),
              ),
            ),
            SliverToBoxAdapter(child: _buildSafetyNotice()),
            ...
            child: Padding(
              ...
              child: Row(children: [
                ...
                Text(context.tr('tide.swipeToExplore'),
                    style: GoogleFonts.inter(color: _txt(0.35), fontSize: 11)),
              ]),
            )));
  }
  ...
  Widget _buildSelectedBanner() {
    final selected = _data.hourlyCards[_selectedHourIndex];
    final windText =
        selected.windSpeed > 0 ? '${selected.windSpeed}km/h' : '--';
    return AnimatedSwitcher(
      ...
      child: Padding(
        ...
        child: _glassPanel(
          ...
          borderColor:
              _levelColor(selected.activityLevel).withValues(alpha: 0.55),
          child: Row(children: [
            Expanded(
                child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                  Text(context.tr('tide.selectedHour'),
                      style: GoogleFonts.inter(color: _txt(0.4), fontSize: 11)),
                  ...
                  Text(
                      'Marée ${selected.tideTrend} · ${selected.tideHeight.toStringAsFixed(2)} m · $windText ${selected.windDirection}',
                      style:
                          GoogleFonts.inter(color: _txt(0.58), fontSize: 11)),
                ])),
            Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
              Text(
                selected.waveHeight > 0
                    ? '${selected.waveHeight.toStringAsFixed(1)} m'
                    : '--',
                style: GoogleFonts.inter(
                  color: _accent,
                  fontSize: 23,
              ...
              child: Row(children: [
                Container(
                    ...
                    decoration: BoxDecoration(
                        ...
                        borderRadius: BorderRadius.circular(2))),
                const SizedBox(width: 8),
                Text(context.tr('tide.conditions'),
                    style: GoogleFonts.inter(
                        color: _txt(0.9),
                        fontSize: 14,
                        fontWeight: FontWeight.w600)),
                const Spacer(),
                Text(_data.hourlyCards[_selectedHourIndex].label,
                    style: GoogleFonts.inter(
  ...
  Widget _buildConditionMoon(tm.MoonInfo info) => _ConditionCard(
      animation: _ctrl,
      delayIndex: 7,
      icon: '🌙',
      title: context.tr('tide.moonPhase'),
      value: info.phaseName,
      subtitle: 'Calcul astronomique',
      child: RepaintBoundary(
          child: SizedBox(
              width: 40,
              height: 40,
              child: CustomPaint(painter: _MoonPainter(phase: 0.0)))));
  Widget _buildConditionWind(tm.WindInfo info) => _ConditionCard(
      animation: _ctrl,
      delayIndex: 8,
      icon: '💨',
      title: context.tr('tide.wind'),
      value: info.speed > 0 ? '${info.speed} km/h ${info.direction}' : '--',
      subtitle: info.gust > 0
          ? 'Rafales ±${info.gust} km/h'
      ...
      child: RepaintBoundary(
          child: SizedBox(
              ...
              height: 40,
              child:
                  CustomPaint(painter: _WindLinesPainter(animation: _ctrl)))));
  Widget _buildConditionWaves(tm.WaveInfo info) => _ConditionCard(
      animation: _ctrl,
      delayIndex: 9,
      icon: '🌊',
      title: context.tr('tide.waves'),
      value: info.height > 0
          ? '${info.height.toStringAsFixed(1)}m / ${info.period}s'
          : '--',
      subtitle: info.swell,
      child: RepaintBoundary(
          child: SizedBox(
              ...
              height: 40,
              child:
                  CustomPaint(painter: _WaveSinePainter(animation: _ctrl)))));
  Widget _buildConditionSun(tm.SunTimes info) => _ConditionCard(
      animation: _ctrl,
      delayIndex: 10,
      icon: '☀️',
      title: context.tr('tide.sun'),
      value: '${info.sunrise} — ${info.sunset}',
      subtitle: 'Horaires approximatifs',
      child: RepaintBoundary(
          child: SizedBox(
              width: 40,
              height: 40,
              child: CustomPaint(painter: _SunArcPainter(animation: _ctrl)))));

  Widget _buildConditionWindForSelected() {
    final selected = _data.hourlyCards[_selectedHourIndex];
    return _buildConditionWind(
      tm.WindInfo(
        speed: selected.windSpeed,
  ...
  Widget _buildConditionWavesForSelected() {
    final selected = _data.hourlyCards[_selectedHourIndex];
    return _buildConditionWaves(
      tm.WaveInfo(
        height: selected.waveHeight,
        period: selected.wavePeriod,
        swell: selected.waveHeight > 0
            ? 'Prévision marine'
            : 'Données indisponibles',
      ),
    );
              ...
              child: Row(children: [
                Container(
                    ...
                    decoration: BoxDecoration(
                        ...
                        borderRadius: BorderRadius.circular(2))),
                const SizedBox(width: 8),
                Text(context.tr('tide.tideEvents'),
                    style: GoogleFonts.inter(
                        color: _txt(0.9),
                        fontSize: 14,
                        fontWeight: FontWeight.w600))
  ...
  Widget _buildTideEventCard(tm.TideEvent event) {
    ...
    return _glassPanel(
      ...
      borderRadius: BorderRadius.circular(16),
      borderColor: (isHigh ? _accent : _red).withValues(alpha: 0.48),
      child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Row(children: [
              Icon(isHigh ? Icons.water : Icons.water_drop_outlined,
                  color: isHigh ? _accent : _red, size: 14),
              const SizedBox(width: 4),
              Flexible(
                  child: Text(
                      context
            ...
            const SizedBox(height: 4),
            Text(_formatDecimalTime(event.time),
                style: GoogleFonts.inter(
                    color: _txt(1.0),
                    fontSize: 16,
                    fontWeight: FontWeight.w800)),
            const SizedBox(height: 1),
            Text('${event.height.toStringAsFixed(2)} m MSL',
                style: GoogleFonts.inter(color: _txt(0.58), fontSize: 10)),
          ]),
    );
  }
  ...
  Widget _buildSafetyNotice() {
    return Padding(
      ...
      child: _glassPanel(
        ...
        borderColor: _amber.withValues(alpha: 0.52),
        padding: const EdgeInsets.all(12),
        child: Row(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            ...
            Expanded(
              child: Text(
                'Prévisions indicatives issues d’un modèle marin. La précision '
                'est limitée près des côtes. Ne pas utiliser pour la navigation '
                'et vérifier les bulletins et horaires officiels locaux.',
                style: GoogleFonts.inter(
                  color: _txt(0.74),
                  fontSize: 10,
                  height: 1.4,
...
class _HourlyCardWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final waveText =
        card.waveHeight > 0 ? '${card.waveHeight.toStringAsFixed(1)} m' : '--';
    return GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          child:
                    ...
                    Container(
                        ...
                        decoration: BoxDecoration(
                            ...
                            border: Border.all(
                                ...
                                width: 0.5)),
                        child: Text(context.tr('tide.nowShort'),
                            style: GoogleFonts.inter(
                                color: _green,
                                fontSize: 7,
                                fontWeight: FontWeight.w800))),
            ...
            const Spacer(),
            Text(
              '${card.tideTrend == 'montante' ? '↑' : '↓'} ${card.tideHeight.toStringAsFixed(2)} m',
              style: GoogleFonts.inter(
                  color: color, fontSize: 10, fontWeight: FontWeight.w700),
              textAlign: TextAlign.center,
              maxLines: 1,
            ),
            Text('$waveText · ${card.wavePeriod} s',
                style: GoogleFonts.inter(color: _txt(0.54), fontSize: 8.5),
                textAlign: TextAlign.center,
                maxLines: 1,
                overflow: TextOverflow.ellipsis),
          ]),
...
class _FishIconState extends State<_FishIcon>
    with SingleTickerProviderStateMixin {
  late final AnimationController _ctrl;
  late final Animation<double> _swim, _pulse, _glow;
  @override
  void initState() {
    super.initState();
    _ctrl = AnimationController(
      ...
      ..repeat(reverse: true);
    _swim = Tween<double>(begin: -0.12, end: 0.12)
        .animate(CurvedAnimation(parent: _ctrl, curve: Curves.easeInOut));
    _pulse = Tween<double>(begin: 0.92, end: 1.08).animate(CurvedAnimation(
        parent: _ctrl,
        curve: const Interval(0.0, 0.5, curve: Curves.easeInOut)));
    _glow = Tween<double>(begin: 0.2, end: 0.6).animate(CurvedAnimation(
        parent: _ctrl,
        curve: const Interval(0.0, 0.5, curve: Curves.easeInOut)));
  }
  ...
  Widget build(BuildContext context) {
    ...
    final baseScale =
        widget.level == 'high' ? 1.0 : (widget.level == 'mid' ? 0.7 : 0.45);
    return AnimatedBuilder(
        animation: _ctrl,
        builder: (context, child) {
...
class _FishBodyPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawPath(
        Path()
          ..moveTo(w * 0.15, h * 0.5)
          ..quadraticBezierTo(w * 0.15, h * 0.15, w * 0.5, h * 0.15)
          ..quadraticBezierTo(w * 0.85, h * 0.15, w * 0.85, h * 0.5)
          ..quadraticBezierTo(w * 0.85, h * 0.85, w * 0.5, h * 0.85)
          ..quadraticBezierTo(w * 0.15, h * 0.85, w * 0.15, h * 0.5)
          ..close(),
        Paint()
          ..color = color
        ...
        child: SlideTransition(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                  color: _card,
                  borderRadius: BorderRadius.circular(16),
                  border: Border.all(color: _glassBorder, width: 0.7),
                  boxShadow: [
                    BoxShadow(
                      ...
                      if (subtitle.contains('Influence'))
                        Container(
                            ...
                            decoration: BoxDecoration(
                                ...
                                borderRadius: BorderRadius.circular(8)),
                            child: Text(subtitle.split(' ').last,
                                style: GoogleFonts.inter(
                                    color: _accent,
                                    fontSize: 9,
                                    fontWeight: FontWeight.w600))),
    ...
    canvas.clipPath(Path.combine(
        ...
        Path()
          ..addOval(Rect.fromCircle(
              center: Offset(c.dx + r * 0.4, c.dy), radius: r))));
...
class _WindLinesPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final p = Paint()
      ...
      ..strokeCap = StrokeCap.round;
    for (int i = 0; i < 3; i++) {
      final y = size.height * [0.25, 0.5, 0.75][i];
      canvas.drawLine(
          Offset(size.width * 0.1, y),
          Offset(
              size.width * 0.1 +
                  (size.width * 0.3 + i * size.width * 0.15) * animation.value,
              y),
          p);
    }
  }
...
class _PillCurvePainter extends CustomPainter {
  ...
  static const double _padL = 20.0,
      _padR = 56.0,
      _pillZoneH = 58.0,
      _hourZoneH = 30.0,
      _topMargin = 8.0;
    ...
    for (double x = 0; x <= size.width; x += 1) {
      path.lineTo(
          ...
          size.height / 2 +
              size.height *
                  0.25 *
                  math.sin(2 * math.pi / size.width * x +
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawPath(
        ...
        Paint()
          ...
          ..style = PaintingStyle.stroke
          ..strokeWidth = 2);
    final sp = 0.3 + animation.value * 0.4;
    final a = math.pi * (1 - sp);
    final sun = Offset(c.dx + r * math.cos(a), c.dy - r * math.sin(a));
    canvas.drawCircle(sun, 4, Paint()..color = _amber);
    canvas.drawCircle(
        sun,
        8,
        Paint()
